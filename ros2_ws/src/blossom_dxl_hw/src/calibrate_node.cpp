...
static constexpr uint16_t ADDR_OPERATING_MODE    = 11;   // 3 = Position Mode
static constexpr uint16_t ADDR_LED               = 65;   // 0/1
static constexpr uint16_t ADDR_GOAL_POSITION     = 116;  // 4B
static constexpr uint16_t ADDR_PRESENT_POSITION  = 132;  // 4B

// --- Unit helpers ---
static inline int   deg_to_pos(double deg){
  if (deg < 0) deg = 0;
  if (deg > 360) deg = 360;
  return int(std::round(deg * 4095.0 / 360.0));
}
static inline double pos_to_deg(int pos){ return pos * 360.0 / 4095.0; }

// --- Raw TTY for non-blocking key reads ---
class RawTTY {
public:
  RawTTY() {
    fd_ = ::fileno(stdin);
...
// --- Safe write wrappers that print SDK comm issues ---
static inline void dx_write1(dx::PacketHandler* pkt, dx::PortHandler* port, int id, uint16_t addr, uint8_t val) {
  uint8_t err=0;
  int res = pkt->write1ByteTxRx(port, id, addr, val, &err);
  if (res != 0 || err != 0) {
    std::cerr << "[DXL] write1 id="<<id<<" addr="<<addr<<" val="<<(int)val<<" res="<<res<<" err="<<(int)err<<"\n";
  }
}
static inline void dx_write4(dx::PacketHandler* pkt, dx::PortHandler* port, int id, uint16_t addr, uint32_t val) {
  uint8_t err=0;
  int res = pkt->write4ByteTxRx(port, id, addr, val, &err);
  if (res != 0 || err != 0) {
    std::cerr << "[DXL] write4 id="<<id<<" addr="<<addr<<" val="<<val<<" res="<<res<<" err="<<(int)err<<"\n";
  }
}

class CalibNode : public rclcpp::Node {
public:
  CalibNode() : rclcpp::Node("blossom_calibrate_cpp") {
    // Parameters
    ...
    names_   = this->declare_parameter<std::vector<std::string>>(
                "names", {"base_pan","string_front","string_back_right","string_back_left"});
    ...
    RCLCPP_INFO(get_logger(), "Calibrating on %s @ %d, ids=%s",
                port_.c_str(), baud_, vec_to_str(ids_).c_str());

    // Configure each motor: torque OFF → Position Mode (3) → LED ON → torque ON
    for (auto id64 : ids_) {
      int id = int(id64);
      dx_write1(pkt, port, id, ADDR_TORQUE_ENABLE, 0);
      dx_write1(pkt, port, id, ADDR_OPERATING_MODE, 3);  // Position mode
      dx_write1(pkt, port, id, ADDR_LED, 1);             // Blink LED to confirm writes
      dx_write1(pkt, port, id, ADDR_TORQUE_ENABLE, 1);
    }
    rclcpp::sleep_for(std::chrono::milliseconds(120));    // settle

    run_interactive();

    // Shutdown: torque OFF + LED OFF + close
    for (auto id64 : ids_) {
      int id = int(id64);
      dx_write1(pkt, port, id, ADDR_TORQUE_ENABLE, 0);
      dx_write1(pkt, port, id, ADDR_LED, 0);
    }
    port->closePort();
  }
...
private:
  ...
  double read_deg(int id){
    ...
    if (err != 0) std::cerr << "[DXL] read4 err id="<<id<<" err="<<(int)err<<"\n";
    return pos_to_deg(int(raw));
  }
  void set_deg(int id, double deg){
    uint32_t ticks = (uint32_t)deg_to_pos(deg);
    dx_write4(pkt, port, id, ADDR_GOAL_POSITION, ticks);
  }

  static std::string fmt1(double v){
    std::ostringstream o; o.setf(std::ios::fixed); o.precision(1); o<<v; return o.str();
  }
    ...
    for(size_t i=0;i<ids_.size();++i){
      ...
      while(rclcpp::ok()){
        ...
        if(c=='<'||c=='>'||c==','||c=='.'||c=='['||c==']'){
          ...
          table[i].neutral = cur; std::cout << "\n  • Neutral set.\n";
        } else if(c=='1'){
          table[i].minv = cur;   std::cout << "\n  • Min set.\n";
        } else if(c=='2'){
          table[i].maxv = cur;   std::cout << "\n  • Max set.\n";
        } else if(c=='g'){
          if(table[i].neutral){ tgt = *table[i].neutral; set_deg(id, tgt); }
          std::cout << "\n  • Go neutral.\n";
        } else if(c=='t'){
          dx_write1(pkt, port, id, ADDR_TORQUE_ENABLE, 1); std::cout << "\n  • Torque ON.\n";
        } else if(c=='o'){
          dx_write1(pkt, port, id, ADDR_TORQUE_ENABLE, 0); std::cout << "\n  • Torque OFF.\n";
        } else if(c=='s'){
          std::cout << "\nSaved & next.\n"; break;
        } else if(c=='q'){
